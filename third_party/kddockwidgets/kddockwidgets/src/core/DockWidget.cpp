...
void DockWidget::moveToCenter()
{
    if (auto fw = floatingWindow()) {
        Core::Screen::Ptr screen = Platform::instance()->primaryScreen();
        if (screen == nullptr) {
            return;
        }
        auto geo = fw->geometry();
        geo.moveCenter(screen->geometry().center());
        fw->setSuggestedGeometry(geo, SuggestedGeometryHint_PreserveCenter);
    }
}
